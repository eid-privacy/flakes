...
let
  ...
  hashes = (import ./hashes.nix).nargo-t256;

  revHashes = hashes.${rev} or { };

  resolvedSrcHash = if srcHash != null then srcHash else revHashes.srcHash or lib.fakeHash;
  resolvedCargoHash = if cargoHash != null then cargoHash else revHashes.cargoHash or lib.fakeHash;
